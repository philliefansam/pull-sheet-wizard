...
function processScannedFiles() {
  ...
  function getPrevSheetData(key, fileName, idx) {
    ...
    if (match && match.is_modified) {
      return {
        is_modified: true,
        manual_final_length: match.manual_final_length,
        manual_final_width: match.manual_final_width
      };
    }
    return {};
  }
    ...
    ${mat.sheets.map((sheet, idx) => {
      ...
      return `
        <div class="sheet-dimension-item" style="display: flex; justify-content: space-between; font-size: 12px; color: var(--text-secondary); align-items: center;">
          ...
          <span>Raw: <strong>${rawX.toFixed(1)}" x ${rawY.toFixed(1)}"</strong> | Final: <strong>${finX.toFixed(1)}" x ${finY.toFixed(1)}"${tagText ? ' ' + tagText : ''}</strong>${actionBtnHtml}</span>
        </div>
      `;
    }).join('')}
  `;
